...
mod tests {
    ...
    fn h_in_insert_mode_types_the_letter_h() {
        let state = new_state(vec![node(&format!("a{PAD}"))], Mode::Insert, Some(Path { head: 0, tail: vec![] }));
    ...
    fn typing_appends_to_the_selected_box_label() {
        let state = new_state(vec![node(&format!("h{PAD}"))], Mode::Insert, Some(Path { head: 0, tail: vec![] }));
    ...
    fn space_and_tilde_are_printable() {
        ...
        let state = new_state(vec![node(PAD)], Mode::Insert, Some(Path { head: 0, tail: vec![] }));
        let result = handle_key(state, "~");
    ...
    fn backspace_drops_the_last_character_and_is_a_no_op_when_empty() {
        let state = new_state(vec![node(&format!("hi{PAD}"))], Mode::Insert, Some(Path { head: 0, tail: vec![] }));
